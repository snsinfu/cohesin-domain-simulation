        ...
        records.append(
            {
                **r.sweep_data,
                ...
                "mean_cohesins": r.mean_cohesins,
                "mean_captures": r.mean_captures,
                "mean_loop_size": r.mean_loop_size,
            }
        )
...
def compute_metrics(filename: str, frames: slice) -> Metrics:
    ...
    for snap in snapshots:
        loaded_sites = snap.loop_captures[:, 0]
        captured_sites = snap.loop_captures[:, 1]
        capture_selection = (loaded_sites != captured_sites)

        cohesin_counts.append(len(loaded_sites))
        capture_counts.append(capture_selection.sum())
        loop_sizes.extend(np.abs(loaded_sites - captured_sites)[capture_selection])

    mean_cohesins = np.mean(cohesin_counts)
    mean_captures = np.mean(capture_counts)
...
def load_snapshots(store: h5py.Group, frames: slice) -> list[Snapshot]:
    ...
    for step in store[".steps"][frames]:
        ...
        loop_captures = sample["captures/sites"][:]
        snapshots.append(Snapshot(positions=positions, associations=associations, loop_captures=loop_captures))
    return snapshots
