...
def generate_domain(
    ...
    loops = generate_island_loops(
        ...
        require_peripheries=loop_require_peripheries,
    )
...
def generate_island_loops(
    ...
    count: int,
    min_size: int,
    require_peripheries: bool,
) -> list[Pair]:
    loops = []

    if require_peripheries:
        loops.extend(generate_peripheral_loops(islands, min_size))
...
def generate_peripheral_loops(
    islands: list[Interval],
    min_size: int,
) -> list[Pair]:
    loops = []

    islands = sorted(islands, key=lambda island: island.start)

    # Loop from the left periphery
    while True:
        island_1 = islands[0]
        island_2 = random.choice(islands[1:])
        i = random.randint(island_1.start, island_1.end - 1)
        j = random.randint(island_2.start, island_2.end - 1)
        pair = Pair(i, j)
        if loop_size(pair) >= min_size:
    ...
    if island_2 is islands[-1]:
        return loops

    # Loop from the right periphery
    while True:
        island_1 = random.choice(islands[:-1])
        island_2 = islands[-1]
        i = random.randint(island_1.start, island_1.end - 1)
        j = random.randint(island_2.start, island_2.end - 1)
        pair = Pair(i, j)
