...
def main(
    *,
    ...
    master_random = np.random.Generator(np.random.PCG64(sweep_seed))
    config_randoms = master_random.spawn(sweep_count)
    ...
    for config_id in range(sweep_count):
        config, resolved_data = resolve_config(
            ...
            **sweep_params,
            **extra_params,
            random=config_randoms[config_id],
        )
    ...
    *,
    ...
    domains = [
        generate_domain(
            ...
            acetylation_level,
            acetylation_error,
            random=domain_random,
        )
        for domain_random in random.spawn(domain_count)
    ]
    ...
    resolved_data["domain_acetylation_levels"] = acetylation_levels

    resolved_data["domain_acetylation_islands"] = [
        [(island.start, island.end) for island in domain.islands]
        for domain in domains
    ]

    resolved_data["domain_loops"] = [
        [(loop.site_1, loop.site_2) for loop in domain.loops]
        for domain in domains
    ]
...
def generate_domain(
    ...
    # Make acetylation and loop generations independent for making comparison
    # between perturbed and control cases easier.
    acetyl_random, loop_random = random.spawn(2)

    while True:
        pattern = generate_acetylation_pattern(
            acetylation_level,
            domain_size,
            island_size=acetylation_island,
...
def generate_island_loops(
    ...
    def _check_crossing(pair: Pair, loops: list[Pair]) -> bool:
        return any(
...
def generate_acetylation_pattern(
    ...
    *,
    ...
    if phi > 0.5:
        inverse_pattern = generate_acetylation_pattern(
            1 - phi,
            length,
