...
def main(
    *,
    ...
    sweep_set = list(sweep_params(sweep_spec))
    randoms = master_random.spawn(len(sweep_set))

    print(f"Generating {len(sweep_set)} configs")
    ...
    for config_id, swept_params in enumerate(sweep_set):
        ...
        config["@meta"]["sweep_params"] = swept_params
...
def sweep_params(specs: dict[str, list[any]]):
